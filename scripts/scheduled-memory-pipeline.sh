...
"${CLI[@]}" pipeline gate --skip-ingest --max-per "${MARROW_LLM_MAX_PER:-${ASD_LLM_MAX_PER:-5/24h}}"

PARSED_RETENTION_AGE_DAYS="${MARROW_PARSED_RETENTION_OLDER_THAN_DAYS:-${ASD_PARSED_RETENTION_OLDER_THAN_DAYS:-30}}"
PARSED_MAX_TOTAL_BYTES="${MARROW_PARSED_MAX_TOTAL_BYTES:-${ASD_PARSED_MAX_TOTAL_BYTES:-1073741824}}"
...
if [[ -n "${OPENROUTER_API_KEY:-}" ]]; then
  ...
  REVIEW_OUTPUT="$("${CLI[@]}" quality review-learnings --if-new --max-per "${MARROW_LLM_MAX_PER:-${ASD_LLM_MAX_PER:-5/24h}}")"
  printf '%s\n' "$REVIEW_OUTPUT"
  BATCH_PATH="$(node -e '
...
if [[ "${MARROW_ENABLE_COMPACTION:-${ASD_ENABLE_COMPACTION:-0}}" == "1" ]]; then
  COMPACTION_AGE_DAYS="${MARROW_COMPACTION_OLDER_THAN_DAYS:-${ASD_COMPACTION_OLDER_THAN_DAYS:-30}}"
