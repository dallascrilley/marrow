...
marrow's deterministic pipeline (ingest → summarize → extract instincts → render
...
1. **Reinforcement never accumulated.** Recompute/replay reset every instinct's
   ...
   `MEMORY.md` across 259 projects (`marrow stats` → `reachability.reachable=2`).
...
- **`marrow recall` is the read-back surface.** It resolves the project id from the
  current working directory (git-remote hash, `.marrow-project-key` declared key, or
...
- **Delivery is a SessionStart hook.** `marrow recall` is wired into the
  agent harness as a SessionStart hook, registered in the user's own hook
  directory alongside any other hooks configured there. It resolves the project
  from the SessionStart payload `cwd`, runs `marrow recall`, and emits the result as
  ...
  hook can add `codex`/`cursor` event blocks later.
...
- A new session in any of the 105 memoried projects now receives its curated
  ...
  1,034 bytes / 4 bullets for `media-studio`). Read-back is real, not
  aspirational.
