...
- [x] (2026-06-30) U10. LLM-judged global promotion fast-path — PR #101 (td-fa381c). Recalibrates the promotion economics the diagnosis flagged as the core value blocker: the deterministic gate's "same id in ≥2 projects @ ≥0.8" never fires (U5 spike: ~0 cross-project id overlap), so `detectGlobalJudgeCandidates` surfaces single-project instincts at ≥0.7 confidence (the live cliff — ~257 qualify) and `judge.ts` asks deepseek/deepseek-v4-flash whether each is globally applicable; approved → `scope: global` store → `_global/MEMORY.md` (U7) → recall (U1). `promote judge` command, bounded by `--limit`/count+USD budgets, fails open without a key. **Live-proven:** real populate judged 30 → promoted 16, rejected 14, $0.0024; recall now leads every session with 16 global instincts. Headline metric: "2 instincts a future agent will ever see" → 16 (243 candidates remain). New unit beyond the original U1–U9.
