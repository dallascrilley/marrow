...
import { reviewProjectLearningsWithOpenRouter } from "../pipeline/llm-learning-review.js";
...
export async function executeQualityReviewLearnings(
	...
	for (const session of sessions) {
		...
		const learnings = await readProjectLearnings(
			session.project_key,
			session.session_id,
		);
		...
		JSON.stringify(
			{
				...
				rewrites: reviewed.filter((entry) => entry.verdict === "rewrite")
					.length,
...
function parseIntegerOption(
	...
	if (!Number.isInteger(parsed) || parsed < 0) {
		throw new Error(
			`quality review-learnings ${flag} requires a non-negative integer`,
		);
	}
...
async function readProjectLearnings(
	projectKey: string,
	sessionId: string,
): Promise<Learning[]> {
	try {
