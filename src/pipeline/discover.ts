...
import {
	upsertSourceSession,
	type UpsertSourceSessionResult,
} from "../db/ledger.js";
...
export async function runDiscoverPhase(
	input: DiscoverPhaseInput,
): Promise<DiscoverPhaseResult> {
	const discovery = await runAdapterDiscovery(input);
	const sinceTimestamp = input.since
...
function toSourceSession(
	...
	const sessionId =
		...
		basename(transcript.sourcePath, extname(transcript.sourcePath));
