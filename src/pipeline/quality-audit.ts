...
export async function auditQuality(
	...
	for (const sourceSession of selectedSessions) {
		const candidate = getDeletionCandidateBySessionId(
			database,
		...
		if (candidate === null) {
			...
			blockedReasons[candidate.reason] =
				(blockedReasons[candidate.reason] ?? 0) + 1;
		}
		...
		sessions.push({
			...
			safe_to_delete:
				candidate === null ? null : candidate.safe_to_delete === 1,
...
async function readSummary(sessionId: string): Promise<SummaryReadResult> {
	try {
		const contents = await readFile(
			getSessionSummaryJsonPath(sessionId),
			"utf8",
		);
		return {
			error: null,
...
function collectSessionIssues(
	...
	if (
		/\b(?:let me|i(?:'|’)ll|i need to|checking|exploring)\b/i.test(summaryText)
	) {
...
async function readKnowledgeArtifactState(
	sourceSession: SourceSessionRow,
): Promise<{
	project: boolean;
	user: boolean;
...
function isLowSignalSummary(summary: Summary): boolean {
	return (
		!hasUsefulSummarySignal(summary) &&
		summary.next_step === "No explicit next step recorded."
	);
}
...
function looksLikeCompletedOutcome(value: string): boolean {
	return (
		/\b(?:done|completed|implemented|fixed|resolved|merged|pushed)\b/i.test(
			value,
		) &&
		/\b(?:verified|tests? pass(?:ed)?|all checks passed|0 failures)\b/i.test(
			value,
		)
	);
}
...
async function countProjectLearnings(
	sourceSession: SourceSessionRow,
): Promise<number> {
	try {
		const contents = await readFile(
...
function buildLearningDistribution(
	sessions: readonly QualityAuditSession[],
