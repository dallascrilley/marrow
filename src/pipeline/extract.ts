			...
			extractUserPreferenceCandidates(
				...
				(candidate, index) =>
					createLearning({
						...
						promotionBasis:
							"Explicit user instruction captured in the source prompt.",
						scope: "user",
...
function hasSameTurnVerifiedFix(
	...
	return (
		turnEvents.some(
			(event) =>
				event.type === "fix" && !looksLikeProcessNarration(event.summary),
		) &&
		turnEvents.some(
			(event) =>
				event.type === "verification" &&
				isUsefulVerificationText(event.summary),
		)
	);
}
function toProjectEventCandidate(
	...
	if (
		event.type === "failure" &&
		looksLikeCompletionNotFailure(event.summary)
	) {
		return null;
	}
	...
	if (
		event.type === "decision" &&
		(looksLikeExplanationNotDecision(event.summary) ||
			looksLikeNonDurableSummary(event.summary))
	) {
		return null;
	}
	...
	if (completionStatement !== null) {
		return {
			...
			promotionBasis:
				"Derived from a verified completion outcome in the reduced transcript.",
			sourceRefs: [
	...
	switch (event.type) {
		...
		case "failure":
			return {
				...
				promotionBasis:
					"Derived from a concrete failure event in the reduced transcript.",
				sourceRefs: [
					createSourceRef(sourceSession, {
						eventId: event.event_id,
						line: event.source_offsets.start_line,
		...
		case "verification": {
			const verificationCommand = readPayloadString(
				event,
			...
			return {
				...
				promotionBasis:
					"Derived from an explicit verification command captured in the transcript.",
				sourceRefs: [
					createSourceRef(sourceSession, {
						eventId: event.event_id,
						line: event.source_offsets.start_line,
...
function toProjectTurnCandidates(input: {
	...
	const turnEvents = input.events.filter(
		(event) =>
			event.turn_id === input.turn.turn_id && event.confidence !== "low",
	);
	const hasVerifiedFix = hasSameTurnVerifiedFix(
		input.events,
		input.turn.turn_id,
	);
	...
	const verificationEvents = turnEvents.filter(
		(event) =>
			event.type === "verification" && isUsefulVerificationText(event.summary),
	);
	...
	for (const failure of failureEvents) {
		...
		candidates.push({
			...
			promotionBasis:
				"Derived from concrete failure evidence followed by a fix or verification.",
			sourceRefs: sourceRefsForEvents(input.sourceSession, [
				failure,
				resolution,
			]),
			statement,
	...
	if (workflowCommand !== null) {
		...
		candidates.push({
			...
			promotionBasis:
				"Derived from project-specific command usage in the reduced turn.",
			sourceRefs: [
				createSourceRef(input.sourceSession, {
	...
	if (
		...
		candidates.push({
			...
			promotionBasis:
				"Derived from a concrete file path and fix outcome in the same turn.",
	...
	if (prReviewCommand !== null) {
		...
		candidates.push({
			...
			promotionBasis:
				"Derived from PR review command usage in the reduced turn.",
			sourceRefs: [
				createSourceRef(input.sourceSession, {
					turnId: input.turn.turn_id,
				}),
	...
	if (forkSyncCommand !== null) {
		...
		candidates.push({
			...
			promotionBasis:
				"Derived from fork maintenance command usage in the reduced turn.",
			sourceRefs: [
				createSourceRef(input.sourceSession, {
					turnId: input.turn.turn_id,
				}),
	...
	if (crashDiagnostic !== null) {
		candidates.push({
			...
			promotionBasis:
				"Derived from crash diagnostic evidence in the reduced turn.",
			sourceRefs: [
				createSourceRef(input.sourceSession, {
					turnId: input.turn.turn_id,
				}),
	...
	if (specDecision !== null) {
		candidates.push({
			...
			promotionBasis:
				"Derived from spec or architecture discussion in the reduced turn.",
			sourceRefs: [
				createSourceRef(input.sourceSession, {
					turnId: input.turn.turn_id,
				}),
...
function toVerifiedCompletionStatement(event: Event): string | null {
	...
	const doneMatch = stripped.match(
		/\*\*Done:\*\*\s*(.*?)(?:\s+\*\*Verified:\*\*\s*|$)(.*)$/i,
	);
	...
	const command =
		extractCommandFromText(verifiedText) ??
		readPayloadString(event, "verification_command");
	const verifiedClause =
		command === null ? "verified" : `verified with ${formatCommand(command)}`;
...
function extractUserPreferenceCandidates(prompt: string): Array<{
	...
	for (const line of splitPromptLines(prompt)) {
		...
		if (
			/\bprefer\b/i.test(line) ||
			/\balways\b/i.test(line) ||
...
function commandFromEvent(event: Event): string | undefined {
	return (
		readPayloadString(event, "verification_command") ??
		usefulCommandsForEvent(event)[0]
	);
}
...
function isUsefulVerificationText(value: string): boolean {
	if (
		/\b(?:can|will|could|should)\s+verify\b/i.test(value) ||
		/\blet me verify\b/i.test(value)
	) {
		return false;
	}
...
function selectWorkflowCommand(
	...
	return setupCommand !== undefined && isWorkflowPrompt(prompt)
		? setupCommand
		: null;
}
...
function selectPrReviewCommand(
	commands: readonly string[],
	prompt: string,
): string | null {
	if (
		!/\b(?:review|checkout)\b.*\bpr\b|\bpr\b.*\b(?:review|checkout)\b/i.test(
			prompt,
		)
	) {
		return null;
	...
	const worktreeCommand = commands.find((command) =>
		/git worktree (?:add|remove)/i.test(command),
	);
...
function selectForkSyncCommand(
	commands: readonly string[],
	prompt: string,
): string | null {
	...
	const upstreamCommand = commands.find((command) =>
		/git remote add upstream/i.test(command),
	);
	if (upstreamCommand !== undefined) {
		return upstreamCommand;
	}

	const syncCommand = commands.find((command) =>
		/git fetch upstream/i.test(command),
	);
...
function extractCrashDiagnostic(
	...
	const diagnosticCommands = commands.filter((command) =>
		/\b(?:bun -e|node -e|python3? -c|python3? -m pytest|npm test|bun test)\b/i.test(
			command,
		),
	);
	const errorFiles = files.filter(
		(file) =>
			/\.(?:log|crash|err)$/i.test(file) ||
			/(?:crash|error|debug|trace)/i.test(file),
	);
...
function extractSpecDecision(
	...
	const specFiles = files.filter(
		(file) =>
			/\/(?:specs?|docs?|design)\//i.test(file) || /\.(?:md|mdx)$/i.test(file),
	);
...
function looksLikeRawCompletionBlock(value: string): boolean {
	return (
		/\*\*(?:Done|Verified|Changed|Changes|Implemented|Summary of changes):\*\*/i.test(
			value,
...
function looksLikeCompletionNotFailure(summary: string): boolean {
	...
	if (
		normalized.startsWith("--- ## ") &&
		/\bbatch \d+ complete\b/i.test(normalized)
	) {
		return true;
	}
...
function looksLikeExplanationNotDecision(summary: string): boolean {
	...
	if (
		normalized.startsWith("## diff review") ||
		normalized.startsWith("### diff review")
	) {
		return true;
	}
...
function hasExcessiveMarkdownStructure(value: string): boolean {
	const markerMatches = value.match(
		/(?:^|\s)(?:#{1,6}\s+|[-*]\s+|\d+\.\s+|\*\*[^*]+:\*\*)/g,
	);
...
function looksLikeRawWorkflowSummary(value: string): boolean {
	...
	return (
		...
		/\*\*(?:delivered|implemented|summary|summary of changes|changes|verified):\*\*/i.test(
			value,
		) ||
		/\|\s*-{2,}\s*\|/.test(value) ||
...
function dedupeProjectCandidates(
	...
	return uniqueCandidates.sort((left, right) =>
		compareCandidateOrder(left, right),
	);
}
...
function semanticCandidateKey(value: string): string {
	return value
		...
		.replace(/`([^`]+)`/g, "$1")
		.replace(
			/^(?:resolved|fixed|completed|implemented|added|updated|changed)\s+/i,
			"",
		)
		...
		.replace(/;\s*verified[\s\S]*$/i, "")
		.replace(
			/\b(?:the|a|an|to|each|in|by|with|and|or|for|of|on|at|is|are|was|were)\b/g,
			" ",
		)
		.replace(/[^a-z0-9]+/g, " ")
		.replace(/\s+/g, " ")
...
function isDurableCandidate(candidate: ProjectLearningCandidate): boolean {
	...
	if (
		candidate.kind === "verification_rule" &&
		/^Run `?\.\/scripts\/qa`?/i.test(statement)
	) {
		return false;
	}
	...
	if (
		...
		/^(?:The log ends right after|The CSS is fine|Now let me check)/i.test(
			statement,
		)
	) {
		return false;
