...
export async function parseKimiTranscript(
	...
	try {
		for await (const line of lines) {
			...
			const parsedLine = parseKimiWireLine(
				line,
				options.sourcePath,
				lineNumber,
			);
...
function normalizeTranscriptRecord(
	...
	const messageText = contentRedacted
		? null
		: extractMessageText(parsedLine, kind, messageObject);
	const toolInputText =
		kind === "tool_use_stub" || kind === "tool_result_stub"
			? extractKimiToolInputText(messageObject)
			: null;
	const filePaths = uniquePreservingOrder(
		extractFilePaths(parsedLine, messageText, toolInputText),
	);
...
function extractMessageText(
	...
	if (kind === "tool_result_stub") {
		...
		const payload = readValueAtPath(messageObject as JsonRecord, ["payload"]);
		const returnValue = readValueAtPath(payload as JsonRecord, [
			"return_value",
		]);
		if (
			returnValue &&
	...
	const normalized = extractNormalizedText(
		readValueAtPath(parsedLine, ["message", "payload"]),
	);
	if (normalized !== null && !looksRedacted(normalized)) {
		return normalized;
	}
...
function extractKimiToolInputText(
	messageObject: JsonValue | undefined,
): string | null {
	if (
		!messageObject ||
	...
	if (payload && typeof payload === "object" && !Array.isArray(payload)) {
		...
		if (
			...
			!Array.isArray(functionObj)
		) {
			const argsString = readValueAtPath(functionObj as JsonRecord, [
				"arguments",
			]);
