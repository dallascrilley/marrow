...
- **Runtime layout:** ledger SQLite, staging, summaries, knowledge (deterministic + reviewed sidecar), sources/manifests, reviews, archives — see README. Staging alone may live on a pre-created external filesystem.
...
- **External staging:** `AGENT_SESSION_DISTILLERY_STAGING_ROOT` relocates only parsed/reduced intermediates. Configured roots fail closed; migration is copy/hash/receipt based and never deletes its source. S3 is not a live filesystem backend.
...
- **External-volume availability:** every staging reader preflights a configured root. Propagate the override into every launchd/cron environment and clear it to roll back to the unchanged internal source tree.
...
| `AGENT_SESSION_DISTILLERY_STAGING_ROOT` | Pre-created absolute directory for parsed and reduced staging only |
