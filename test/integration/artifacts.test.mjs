...
test("retention blocks deletion until summary, learnings, manifest, and receipt exist", async () => {
  await withRuntimeRoot(async () => {
    ...
    try {
      ...
      assert.match(afterManifest.receipt.reason_if_not, /Retention receipt has not been written yet/);
      ...
      const reportResult = await writeRetentionBatchReport([afterSummary, afterManifest, afterReceipt], "task7");
