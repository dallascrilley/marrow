...
function buildEvents(turnId) {
	return [
		eventSchema.parse({
			...
			summary:
				"Fixed the session writer to emit both markdown and JSON outputs.",
			turn_id: turnId,
			type: "fix",
		}),
		eventSchema.parse({
...
test("retention blocks deletion until summary, learnings, manifest, and receipt exist", async () => {
	await withRuntimeRoot(async () => {
		...
		try {
			...
			const firstCandidate = upsertDeletionCandidate(
				database,
				afterSummary.candidate,
			);
			assert.equal(afterSummary.receipt.safe_to_delete, false);
			assert.match(
				afterSummary.receipt.reason_if_not,
				/No project or user learnings/,
			);
			...
			const expectedUserPath = getUserKnowledgeSessionPath(
				"operator",
				sourceSession.session_id,
			);
			...
			const secondCandidate = upsertDeletionCandidate(
				database,
			...
			const finalCandidate = upsertDeletionCandidate(
				database,
				afterReceipt.candidate,
			);
			...
			const projectKnowledgeLines = (
				await readFile(expectedProjectPath, "utf8")
			)
				.trim()
				.split("\n")
			...
			assert.ok(
				projectKnowledgeLines.every((entry) => entry.confidence !== "low"),
			);
			assert.ok(
				projectKnowledgeLines.some(
					(entry) =>
						entry.statement ===
						"Run npm test when verifying changes in agent-session-distillery.",
				),
			);
			...
			const parsedSummary = JSON.parse(
				await readFile(summaryResult.summaryPath, "utf8"),
			);
			assert.equal(
			...
			const parsedManifest = JSON.parse(
				await readFile(manifestResult.path, "utf8"),
			);
			assert.equal(parsedManifest.source_span.turn_count, 1);
			assert.equal(
				parsedManifest.artifact_paths.retention_receipt_path,
				receiptPath,
			);
