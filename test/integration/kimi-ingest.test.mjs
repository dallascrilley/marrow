...
test("ingest backfill --source kimi processes the smoke fixture end-to-end", async () => {
	...
	try {
		const targetSessionsRoot = join(home, ".kimi", "sessions");
		await mkdir(targetSessionsRoot, { recursive: true });
		await cp(fixtureSessionsRoot, targetSessionsRoot, { recursive: true });
		await cp(fixtureKimiJson, join(home, ".kimi", "kimi.json"));

		const result = runCli(["ingest", "backfill", "--source", "kimi"], {
			HOME: home,
		...
		assert.equal(payload.source, "kimi");
