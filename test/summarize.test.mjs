...
test("deriveTopic skips HTML comment lines in prompts", () => {
  const sourceSession = { ...sourceSessionFixture, session_id: "html-comment-topic" };
  const turns = [
    turnSchema.parse({
      assistant_summary: "Shipped the runtime update.",
      commands_seen: [],
      ended_at: "2026-05-03T23:10:01.000Z",
      files_touched: [],
      index: 0,
      ...
      turn_id: `${sourceSession.session_id}:turn-0000`,
      user_prompt: [
        "<!-- generated by 'hub ship --runtime claude' -->",
        "Ship the runtime config update to production.",
      ].join("\n"),
      verification_seen: false,
    }),
  ];
  ...
  assert.equal(summary.topic, "Ship the runtime config update to production.");
});

test("isLowSignalTopic rejects HTML comment topics", () => {
  assert.ok(isLowSignalTopic("<!-- generated by 'hub ship --runtime claude' -->"));
});

test("deriveTopic never falls back to a harness-noise line as the topic", () => {
  const sourceSession = {
    ...sourceSessionFixture,
    project_key: "demo",
    session_id: "comment-only-prompt",
  };
  const turns = [
    turnSchema.parse({
      assistant_summary: "Patched the retention gate to require receipts before deletion.",
      commands_seen: [],
      ended_at: "2026-05-03T23:10:01.000Z",
      files_touched: [],
      index: 0,
      ...
      tool_stub_count: 0,
      turn_id: `${sourceSession.session_id}:turn-0000`,
      user_prompt: "<!-- generated by 'hub ship --runtime claude' -->\n<!-- do not edit -->",
      verification_seen: false,
    }),
  ];
