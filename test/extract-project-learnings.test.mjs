...
test("does not promote process-only file-scoped fix events", () => {
  ...
  const firstTurn = turn({
    files_touched: [
      "/Users/operator/.hub/artifacts/skills/ce-ideate/source/original/SKILL.md",
    ],
...
test("promotes spec decision from speckit prompt and spec files", () => {
  const source = sourceSession({ project_key: "hub" });
  const firstTurn = turn({
    files_touched: ["/Users/operator/.hub/specs/004-hub-skills-cli-wrapper/spec.md"],
...
test("sanitizes bold emphasis and framing from decision learning statement", () => {
  const source = sourceSession({ project_key: "example-studio" });
  const firstTurn = turn();
  const events = [
    event(
      firstTurn.turn_id,
...
test("does not derive workflow from prompt-instruction turn", () => {
  ...
  const firstTurn = turn({
    commands_seen: [],
    files_touched: [
      "example-studio/trigger-real-video-validation/docs/specs/2026-03-30-cloud-mode-design.md",
    ],
