...
test("promotes same-turn fix and verification into project learning", () => {
	...
	const events = [
		event(
			firstTurn.turn_id,
			"fix",
			"Updated Vitest config to use worker threads and happy-dom.",
			{
		...
		event(
			firstTurn.turn_id,
			"verification",
			"Verification noted: All 92 tests pass.",
			{
...
test("promotes error resolution when failure is followed by a fix", () => {
	...
	const events = [
		event(
			firstTurn.turn_id,
			"failure",
			"Import failed with ENOENT while loading shared/logging.",
			{
...
test("promotes workflow learning from project-specific commands", () => {
	...
	const firstTurn = turn({
		...
		user_prompt:
			"@.cursor/worktrees.json develop a worktree setup script for this project",
	});
...
test("promotes file-scoped implementation learning from concrete fix evidence", () => {
	...
	const events = [
		event(
			firstTurn.turn_id,
			"fix",
			"Resolved by adding `import psycopg.rows`.",
			{
				payload_small: {
	...
	assert.ok(
		learnings.project.some(
			(learning) =>
				learning.statement ===
				"In shared/db_sqlite.py, resolved by adding `import psycopg.rows`.",
		),
	);
});
...
test("does not promote process-only future verification text", () => {
	...
	const events = [
		event(
			firstTurn.turn_id,
			"verification",
			"I can verify after I finish checking the repo.",
			{
				payload_small: {
...
test("promotes crash diagnostic from fix prompt and diagnostic commands", () => {
	...
	const firstTurn = turn({
		...
		files_touched: ["/Users/operator/.pi/agent/pi-crash.log"],
...
test("promotes spec decision from speckit prompt and spec files", () => {
	...
	const firstTurn = turn({
		files_touched: [
			"/Users/operator/.hub/specs/004-hub-skills-cli-wrapper/spec.md",
		],
...
test("does not promote basic git init as project learning", () => {
	...
	const firstTurn = turn({
		...
		user_prompt:
			'git add --all && git commit -m "chore: initial commit" && gh repo create',
	});
...
test("normalizes duplicated fixed prefixes and what-changed clauses", () => {
	...
	const events = [
		...
		event(
			firstTurn.turn_id,
			"verification",
			"Verification noted: `cargo check` passes.",
			{
				event_id: `${firstTurn.turn_id}:verification:000002`,
				payload_small: {
					matched_rule: "verified",
...
test("normalizes completion workflow summaries before promotion", () => {
	...
	const events = [
		...
		event(
			firstTurn.turn_id,
			"verification",
			"Verification noted: `./scripts/qa` passes.",
			{
				event_id: `${firstTurn.turn_id}:verification:000002`,
				payload_small: {
					matched_rule: "verified",
...
test("dedupes later error-resolution candidates covered by earlier verified workflow", () => {
	...
	const events = [
		...
		event(
			secondTurn.turn_id,
			"fix",
			"All 91 tests pass. Run full QA: `./scripts/qa`.",
			{
