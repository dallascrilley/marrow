	...
	await withRuntimeRoot(async () => {
		...
		try {
			...
			await writeSessionSummary({
				...
				project_learnings: [
					"Verified completion outcomes are promoted conservatively.",
				],
				session_id: "good-session",
			...
			upsertDeletionCandidate(database, {
				...
				reason:
					"no_durable_learnings: No project or user learnings have been written yet.",
				safeToDelete: false,
		...
		try {
			...
			for (const [index, count] of counts.entries()) {
				...
				await writeSessionSummary({
					...
					next_step: "No open next step recorded.",
					project_learnings:
						count === 0 ? [] : [`Summary fallback learning ${index}`],
					session_id: sessionId,
					useful_commands: ["npm test"],
			...
			assert.equal(
				report.learning_distribution.sessions_with_project_learnings,
				3,
			);
			...
			assert.equal(
				report.sessions.find(
					(session) => session.session_id === "distribution-session-3",
