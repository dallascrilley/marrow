...
test("classifies ContentPart as assistant_message", async () => {
  const result = await parseFixture();
  const assistantMessages = result.records.filter((r) => r.kind === "assistant_message");
...
test("classifies ToolResult as tool_result_stub", async () => {
  const result = await parseFixture();
  const toolResults = result.records.filter((r) => r.kind === "tool_result_stub");
  assert.equal(toolResults.length, 2);

  const firstResult = toolResults[0];
  assert.ok(firstResult.messageText.includes("osint") || firstResult.messageText.includes("Done"));
...
test("extracts timestamp hints from unix epoch floats", async () => {
  ...
  assert.ok(firstRecord.timestampHint?.includes("Z") || firstRecord.timestampHint?.includes("+"));
});
